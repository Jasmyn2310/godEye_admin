...
export const Dashboard = ({ token, showOnlyPlans = false }: DashboardProps) => {
  ...
  useEffect(() => {
    ...
    const fetchPlans = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/subscriptions/plans`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
      ...
      {!showOnlyPlans && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm relative overflow-hidden group">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 border border-blue-500/50 rounded-2xl p-6 shadow-lg shadow-blue-900/20 relative overflow-hidden group">
              ...
              {plans.length === 0 ? (
                ...
                plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="p-4 text-emerald-400 font-medium">${plan.price}</td>
                    <td className="p-4">
                      {plan.isPopular ? (
